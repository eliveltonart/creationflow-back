...
let DashboardService = class DashboardService {
    ...
    async getAdminDashboard(userId) {
        ...
        const teamMemberIds = new Set(companies.flatMap((c) => c.members.map((m) => m.userId)));
        const activeSprints = companies.flatMap((c) => c.projects.flatMap((p) => p.sprints.map((s) => ({ ...s, projectName: p.name, companyName: c.name }))));
        const overdueTasks = allTasks
            .filter((t) => t.dueDate && new Date(t.dueDate) < now && t.status !== 'DONE')
            .sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime())
            .slice(0, 8)
            .map((t) => {
            const project = companies.flatMap((c) => c.projects).find((p) => p.tasks.some((tk) => tk.id === t.id));
